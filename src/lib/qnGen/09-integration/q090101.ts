...
function case1Gen(case1: number,
	...
	if (case1===0){
		...
		return [math(`\\displaystyle \\int \\frac{1}{x \\ln x} \\; \\mathrm{d}x`), ans, ans, '']
	}
	if (case1===1){
		const xA = `x^{${a1}}`;
...
function case2Gen(case2: number, a2: number, b2: number, n: number): [string, string, string, string] {
	...
	// 0: ln x, 1: x^a + b, 2: trig
	if (case2===0){
		const qn = n===1 ? math(`\\displaystyle \\int \\frac{\\ln x}{x} \\; \\mathrm{d}x`)
			: n>1 ? math(`\\displaystyle \\int \\frac{(\\ln x)^{${n}}}{x} \\; \\mathrm{d}x`)
			: math(`\\displaystyle \\int \\frac{1}{x (\\ln x)^{${-n}}} \\; \\mathrm{d}x`);
	...
	if (case2===1){
		...
		const qn = n > 0 ? math(`\\displaystyle \\int ${fPrime} (${fx})^{${n}} \\; \\mathrm{d}x`)
			: math(`\\displaystyle \\int \\frac{${fPrime}}{(${fx})^{${-n}}} \\; \\mathrm{d}x`);
		let den = a2*(n+1);
		let multiple = 1;
		if (den < 0){
			multiple = -1;
...
function case3Gen(case3: number, a3: number, b3: number, signCase3: number): [string, string, string, string] {
	...
	if (case3===1){
		...
		const qn = math(`\\displaystyle \\int ${fPrime} \\; e^{${fx}} \\; \\mathrm{d}x`);
